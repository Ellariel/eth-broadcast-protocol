...
class Node:
    ...
    def __init__(self,
        ...
        #self.model = None
        #if self.is_mining and self.env.data['use_agent']:
        #    self.model = Agent(node=self, weights=self.env.data['use_agent'])
        self.model = self.env.data['use_agent'] if self.is_mining and self.env.data['use_agent'] else None
    ...
    def connect(self, nodes: list):
        ...
        if 'connections' not in self.env.data:
            self.env.data['connections'] = {}
        self.env.data['connections'][self.address] = [k for k, v in self.active_sessions.items()]           
        ...
        while True:
            ...
            if envelope.msg['id'] == 'block_bodies':
                ...
                for block_hash, _ in envelope.msg['block_bodies'].items():
                    initial_time = block_propagation.get(block_hash[:8], None) # Block broadcasting statistics
                    if initial_time is not None:
                        propagation_time = self.env.now - initial_time[0]
                        blocks.update({f'{block_hash[:8]}': (propagation_time, True)})
    ...
    def broadcast(self, msg):
        ...
        connections = [v for k, v in self.active_sessions.items()]
        if self.env.data['randomize_connections']:
            random.shuffle(connections)  
        
        if self.env.data['broadcast_rules'] and msg['id'] in self.env.data['broadcast_rules']:
            rules = self.env.data['broadcast_rules'][msg['id']]
            if self.address in rules:
                broadcast_order = list(rules[self.address])
                if self.env.data['randomize_connections']:
                    random.shuffle(broadcast_order)
                if self.model:
                    broadcast_order = self.model.predict(self, broadcast_order)
                    connections = [self.active_sessions[n] for n in broadcast_order]
                else:
                    connections = [self.active_sessions[n] for n in broadcast_order]
        ...
        for node in connections: # Broadcasting
            ...
            if msg['id'] == 'new_blocks':
                ...
                for block_hash in msg['new_blocks']:
                    blocks.update({f'{block_hash[:8]}': (self.env.now, False)})
                self.env.data['block_propagation'][f'{origin_node.address}_{destination_node.address}'].update(
                    blocks)
                
                if 'block_propagation_final' not in self.env.data:
                    self.env.data['block_propagation_final'] = {}
                if 'created_blocks' not in self.env.data:
                    self.env.data['created_blocks'] = {}

                for k, (v, _) in blocks.items():
                    if k not in self.env.data['block_propagation_final']:
                        self.env.data['block_propagation_final'][k] = list(set([v]))
                    else:
                        self.env.data['block_propagation_final'][k] = list(set(self.env.data['block_propagation_final'][k] + [v]))
                    if k not in self.env.data['created_blocks']:
                        self.env.data['created_blocks'][k] = v
