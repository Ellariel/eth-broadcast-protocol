...
class ETHNode(Node):
    def __init__(self,
        ...
        self.already_requested_headers = {}
    ...
    def _read_envelope(self, envelope):
        ...
        if envelope.msg['id'] == 'new_blocks':
            ...
            #print('WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW')
    ...
    def _receive_new_blocks(self, envelope):
        ...
        for block_hash, block_number in new_blocks.items():
            if self.chain.get_block(block_hash) is None:
                ...
                if self.env.data['verbose']:
                    print(f'{envelope.destination.address} at {time(self.env)}: new block invitation message from {envelope.origin.address}:', block_hash[:8])
        
        if not is_empty(block_numbers):
            lowest_block_number = min(block_numbers)
            
            if not is_empty([k for k in new_blocks.keys() if k not in self.already_requested_headers]):
                self.already_requested_headers.update(new_blocks)
                self.request_headers(
                    lowest_block_number, len(new_blocks), envelope.origin.address)
            else:
                if self.env.data['verbose']:
                    print(f'{envelope.destination.address} at {time(self.env)}: this block already added:', block_hash[:8])
    ...
    def _receive_block_headers(self, envelope):
        ...
        for header in block_headers:
            ...
            if not self.chain.get_block(header.hash):
                #print('block exists')
            ...
            if block_hash in self.temp_headers:
                ...
                if self.chain.add_block(new_block):
                    ...
                    if self.env.data['verbose']:
                        ...
                        print(f'{self.address} at {time(self.env)}: block assembled and added: {block_hash[:8]}')

                    info = self.env.data['block_propagation_final'].get(block_hash[:8], None) # Block broadcasting statistics
                    if info is not None:
                        info['node_counter'] += 1
                        if info['final_time'] is None:
                            info['final_time'] = self.env.now
                        else:
                            info['final_time'] = max(info['final_time'], self.env.now)
                        self.env.data['block_propagation_final'].update({f'{block_hash[:8]}': info})

                    to_broadcast.append(new_block)
