...
class BTCNode(Node):
    ...
    def _receive_new_inv_blocks(self, envelope):
        ...
        new_blocks_hashes = [b for b in new_blocks_hashes if not self.chain.get_block(b)]    ##########
        #print(len(new_blocks_hashes))
        if len(new_blocks_hashes):
    ...
    def _receive_full_block(self, envelope):
        ...
        block_hash = block.header.hash[:8]
        ...
        if self.chain.add_block(block):
            ...
            info = self.env.data['block_propagation_final'].get(block_hash, None) # Block broadcasting statistics
            if info is not None:
                info['node_counter'] += 1
                ...
                self.env.data['block_propagation_final'].update({f'{block_hash}': info})
                
            to_broadcast.append(block)
        ...
        if len(to_broadcast):
            new_blocks_hashes = [b.header.hash for b in to_broadcast]
            new_blocks_msg = self.network_message.inv(new_blocks_hashes, 'block')
            self.env.process(self.broadcast(new_blocks_msg))        
