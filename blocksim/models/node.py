        ...
        while True:
            ...
            if envelope.msg['id'] == 'block_bodies':
                ...
                for block_hash, _ in envelope.msg['block_bodies'].items():
                    info = block_propagation.get(block_hash[:8], None) # Block broadcasting statistics
                    if info is not None and info['final_time'] is None:
                        info['final_time'] = self.env.now
                        block_propagation.update({f'{block_hash[:8]}': info})              
    ...
    def broadcast(self, msg):
        ...
        for node in connections: # Broadcasting
            ...
            if msg['id'] == 'new_blocks':
                block_propagation = self.env.data['block_propagation'][
                    f'{origin_node.address}_{destination_node.address}']

                for block_hash in msg['new_blocks']:   
                    #print(block_hash)  
                    if block_hash[:8] not in self.env.data['created_blocks']:
                        self.env.data['created_blocks'] += [block_hash[:8]]
                                   
                    info = block_propagation.get(block_hash[:8], None) # Block broadcasting statistics
                    if info is None:
                        info = {'initial_time': self.env.now,
                                'final_time': None}
                        block_propagation.update({f'{block_hash[:8]}': info})
                        
                    info = self.env.data['block_propagation_final'].get(block_hash[:8], None)
                    if info is None:
                        info = {'initial_time': self.env.now,
                                'final_time': None,
                                'node_counter' : 1}
                        self.env.data['block_propagation_final'].update({f'{block_hash[:8]}': info})               
